...
def get_classification_metrics(
    y_true: pd.Series, y_pred: pd.Series, y_prob: pd.Series, prefix: str, pos_label: str
) -> Dict[str, float]:
    """
    ...
    :param pos_label: The positive label to use for computing recall and F1 score.
    :return: Classification metrics.
    """
    metrics = {
        f"{prefix}_accuracy": accuracy_score(y_true, y_pred),
        f"{prefix}_recall": recall_score(y_true, y_pred, pos_label=pos_label),
        f"{prefix}_f1": f1_score(y_true, y_pred, pos_label=pos_label),
